...
function changeBackground() {
   ...
   var bColor = this.style.backgroundColor; /* this td tag.looks at style.looks at background color, and puts it in a variable */
	
	if (bColor == "rgb(233, 207, 29)") bColor = "rgb(101, 101, 101)";
	else if (bColor == "rgb(101, 101, 101)") bColor = "white";
	else bColor = "rgb(233, 207, 29)";
		
	this.style.backgroundColor = bColor; /* this td tag.looks at style.looks at background color = assigns it the bColor color figured out in if statement */
...
function swapPuzzle() 
{
   ...
   {
      var title = "Hanjie " + this.value; /* value in HTML button is what the user sees on button */
      var hint = eval(this.id + "Hint"); /* takes id off button concatanates "Hint" and puts it in varible using "eval" */
...
function peek() {
   var markedCells = document.querySelectorAll("#hanjieGrid td.marked"); /* finding all the td tags with a class of "marked" */
   var emptyCells = document.querySelectorAll("#hanjieGrid td.empty");
   for (var i = 0; i < markedCells.length; i++) {
      var cell = markedCells[i];
   ...
   setTimeout("unpeek()", 500); /* 500 = half a second */
}
